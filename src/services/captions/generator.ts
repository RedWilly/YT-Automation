...
import type { AssemblyAIWord, TranscriptSegment } from "../../types/index.ts";
...
export async function generateAssSubtitles(
  ...
  const highlightOutlineWidth = highlightStyle.useBox ? highlightStyle.outlineWidth : captionStyle.outlineWidth;
  ...
  // Use style-configurable margins (with different value for vertical videos)
  const marginV = isVertical ? captionStyle.marginVVertical : captionStyle.marginV;
  const marginL = captionStyle.marginL;
  const marginR = captionStyle.marginR;

  // Use style-configurable text properties
  const alignment = captionStyle.alignment;
  const scaleX = captionStyle.scaleX;
  const scaleY = captionStyle.scaleY;
  const spacing = captionStyle.letterSpacing;
  const bold = captionStyle.bold ? -1 : 0;  // ASS uses -1 for true, 0 for false
...
Style: Default,${captionStyle.fontName},${captionStyle.fontSize},${captionStyle.primaryColor},&H000000FF,${captionStyle.outlineColor},${captionStyle.backgroundColor},${bold},${italic},0,0,${scaleX},${scaleY},${spacing},0,${defaultBorderStyle},${captionStyle.outlineWidth},${captionStyle.shadowDepth},${alignment},${marginL},${marginR},${marginV},1
Style: Highlight,${captionStyle.fontName},${captionStyle.fontSize},${highlightPrimaryColor},&H000000FF,${highlightOutlineColor},${highlightBackColor},${bold},${italic},0,0,${scaleX},${scaleY},${spacing},0,${highlightBorderStyle},${highlightOutlineWidth},${captionStyle.shadowDepth},${alignment},${marginL},${marginR},${marginV},1
    ...
    if (karaokeEnabled) {
      ...
      for (let i = 0; i < group.words.length; i++) {
        ...
        for (let j = 0; j < group.words.length; j++) {
          ...
          const wordText = captionStyle.uppercase ? word.text.toUpperCase() : word.text;
          if (j === i) {
            // Highlight current word
            textParts.push(`{\\rHighlight}${wordText}{\\r}`);
          } else {
            // Normal word
            textParts.push(`{\\rDefault}${wordText}{\\r}`);
          }
        }
      ...
      const text = group.words.map(w => captionStyle.uppercase ? w.text.toUpperCase() : w.text).join(" ");
